...
const createAndSendToken = (user, statusCode, res) => {
  ...
  // In production, only send the cookie over HTTPS
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  // 1) get user based on posetd email
  ...
  try {
    await sendEmail({
      email: user.email,
      subject: 'Your password reset token(Only valid for 19 minutes).',
      message,
    });
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 2) If token is not expired and there is a user for that token, set the new password
  if (!user) {
  ...
  // 3) Update changedPasswordAt property or the user
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  // 2) Check if the user provided password is correct
...
exports.isLoggedIn = async (req, res, next) => {
  ...
  if (req.cookies.jwt) {
    try {
      ...
      // Making the user available in the pug templates, through req.locals
      res.locals.user = currentUser;
      return next();
    } catch (err) {
      return next();
